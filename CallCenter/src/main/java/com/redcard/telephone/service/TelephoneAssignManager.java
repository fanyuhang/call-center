...
public class TelephoneAssignManager extends GenericPageHQLQuery<TelephoneAssign> {
    ...
    public void saveAssign(TelephoneAssign telephoneAssign) {
        ...
        for (int i = 0; i < telephoneAssign.getFldDayNumber(); i++) {
            ...
            telephoneAssignDetail.setFldRecoverStatus(TelephoneRecoverStatusEnum.NOT.getCode());
            ...
            for (String callUserNo : callUserNos) {
                ...
                if (telephoneAssign.getFldSource().equals(Constant.TELEPHONE_SOURCE_IMPORT)) {//来源：导入话务
                    ...
                    for (TelephoneImportDetail telephoneImportDetail : detailList) {
                        ...
                        telephoneTask.setFldRecoverStatus(TelephoneRecoverStatusEnum.NOT.getCode());
    ...
    public void recover(String ids) {
        ...
        List<TelephoneAssign> telephoneAssignList = new ArrayList<TelephoneAssign>();
        List<TelephoneAssignDetail> telephoneAssignDetailList = new ArrayList<TelephoneAssignDetail>();

        for (String id : idsArray) {

            TelephoneAssign telephoneAssign = telephoneAssignDao.findOne(id);
            telephoneAssign.setFldRecoverDate(new Date());
            telephoneAssign.setFldRecoverStatus(TelephoneRecoverStatusEnum.DONE.getCode());
            telephoneAssign.setFldRecoverUserNo(SecurityUtil.getCurrentUserLoginName());

            telephoneAssignList.add(telephoneAssign);

            //1.获取话务分配明细
            List<TelephoneAssignDetail> assignDetailList = telephoneAssignDetailDao.listByAssignId(id);
            ...
            for (TelephoneAssignDetail telephoneAssignDetail : assignDetailList) {
                ...
                telephoneAssignDetailList.add(telephoneAssignDetail);
            }
        }

        if(telephoneAssignList.size()>0){
            telephoneAssignDao.save(telephoneAssignList);
        }
    ...
    public void recover(TelephoneAssign telephoneAssign) {
        ...
        TelephoneAssign oldOne = telephoneAssignDao.findOne(telephoneAssign.getFldId());
        oldOne.setFldRecoverDate(new Date());
        oldOne.setFldRecoverStatus(TelephoneRecoverStatusEnum.DONE.getCode());
        oldOne.setFldRecoverUserNo(SecurityUtil.getCurrentUserLoginName());
        ...
        telephoneAssignDao.save(oldOne);
    }
