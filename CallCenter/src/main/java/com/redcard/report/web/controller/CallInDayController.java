 ...
 */
@Controller
@RequestMapping(value = "/report/callInDay")
public class CallInDayController {
    private Logger logger = LoggerFactory.getLogger(CallInDayController.class);
    @Autowired
    ...
    public String init(String menuNo, Model model) {
        ...
        return "report/callInDay/list";
    }

    private List<CallInDay> getList(String where, GridPageRequest pageRequest) {
        ...
        try {
            ...
            return reportMybatisManager.callInDay(param);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error(e.getMessage());
        }
        return new PageList<CallInDay>();
    }

    @RequestMapping(value = "list")
    @ResponseBody
    public DataResponse<CallInDay> list(String where, GridPageRequest pageRequest) {
        List<CallInDay> pageList = getList(where, pageRequest);
        DataResponse<CallInDay> dataResponse = new DataResponse<CallInDay>(pageList);
    ...
    public AsyncResponse export(String where, String columnNames, String propertyNames, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            ListToExcel<CallInDay> listToExcel = new ListToExcel<CallInDay>(getList(where, null), columnList, propertyList, "sheet1");
