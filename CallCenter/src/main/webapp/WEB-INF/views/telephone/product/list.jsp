...
<script type="text/javascript">
    ...
    $("#formsearch").ligerForm({
        ...
        inputWidth:200,
        fields:[
            {display:"产品名称", name:"fldTitle", newline:true, type:"text", cssClass:"field"},
            ...
            {display:"操作时间", name:"executeDateBegin", newline:true, cssClass:"field", type:"date", attr:{"op":'greaterorequal', field:"fldOperateDate", vt:"date"}},
    ...
    var grid = $("#maingrid").ligerGrid({
        ...
        delayLoad:true,
        columns:[
            { display:"产品名称", name:"fldTitle", type:"text", align:"center", width:300 },
            { display:'状态', name:'fldStatus', align:'center', width: 120,
                render:function (item) {
                    return renderLabel(statusType, item.fldStatus);
                }},
            { display:'操作人', name:'operateName', align:'center'},
            { display:'操作时间', name:'fldOperateDate', align:'center'}
        ], dataAction:'server', toolbar:{}, url:'<c:url value="/telephone/product/list"/>', sortName:'fldOperateDate', sortOrder:'desc',
    ...
    function toolbarBtnItemClick(item) {
        switch (item.id) {
            case "add":
                top.f_addTab(null, '增加公告内容', '<c:url value="/telephone/product/add"/>' + '?menuNo=${menuNo}');
                break;
            case "modify":
                ...
                top.f_addTab(null, '修改公告内容', '<c:url value="/telephone/product/edit"/>' + '?menuNo=${menuNo}&id=' + selected.fldId);
                break;
            case "delete":
    ...
    function f_delete() {
        ...
        if (selected) {
            LG.ajax({
                url:'<c:url value="/telephone/product/delete"/>',
                loading:'正在删除中...',
