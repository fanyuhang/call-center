...
//        var url = p.url || ashxUrl + $.param({ type: p.type, method: p.method });
    ...
    LG.createDefaultButton = function (item,buttons){
        if (!buttons) return;
        ...
        $(":input", form).not(":submit, :reset, :image,:button, [disabled]")
            .each(function () {
                ...
                if (!$(this).hasClass("field")){
                    //chaochao: 对隐藏类型，不根据field判断
                    //Qingtian Li: 过滤没有require属性的hidden类型
                    if(this.type == "hidden"){
                        var require = $(this).attr("require") || "false";
                        if (require == "false")
                            return;
                    }else{
                        return;
    ...
    //附加表单搜索按钮：搜索、高级搜索
    LG.appendSearchButtons = function (form, grid,btn1Container,btn2Container) {
        if (!form) return;
        form = $(form);
        //搜索按钮 附加到第一个li  高级搜索按钮附加到 第二个li
//        var container = $('<ul><li style="margin-right:8px"></li><li></li></ul><div class="l-clear"></div>').appendTo(form);
        var container = $('<li style="margin-right:8px"></li>').appendTo(form.find("ul:last"));
        LG.addSearchButtons(form, grid, btn1Container==false?btn1Container:container, btn2Container==false?btn2Container:container.find("li:eq(1)"));
    };

    //附加表单搜索按钮：搜索
    LG.appendSearchButton = function (form, grid) {
        if (!form) return;
        form = $(form);
//        搜索按钮 附加到第一个li
        var container = $('<ul><li style="margin-right:8px"></li><li></li></ul><div class="l-clear"></div>').appendTo(form);
//        var container = $('<li style="margin-right:8px"></li>').appendTo(form.find("ul:eq(0)"));
    ...
    LG.setFormDefaultBtn = function (cancleCallback, savedCallback) {
        ...
        if (savedCallback) {
            buttons.push({ text:'保存', onclick:function(){
                $.ligerDialog.confirm('请确认是否保存？', function (yes) {
                    if (yes) {
                        savedCallback();
                    }
                });
    ...
    //快速设置表单底部默认的按钮:保存、取消
    LG.setFormDefaultBtn = function (cancleCallback, savedCallback, isConfirm) {
        //表单底部按钮
        var buttons = [];
        if (cancleCallback) {
            buttons.push({ text:'取消', onclick:cancleCallback });
    ...
    LG.validate = function (form, options) {
        ...
        options = $.extend({
            ...
            success:function (lable) {
                ...
                var element = $("#" + lable.attr("for").replace( /(:|\.|\[|\])/g, "\\$1" ));
                if (element.hasClass("l-textarea")) {
    ...
    /*LG.loadToolbar = function(grid,toolbarBtnItemClick) {
     rootPath = "/static/ligerUI"
     var data = [
     {"BtnID":97,"BtnName":"新增","BtnNo":"add","BtnClass":null,"BtnIcon":"/icons/silkicons/add.png","BtnScript":null,"MenuNo":"BaseManageProducts","InitStatus":null,"SeqNo":null},
     {"BtnID":98,"BtnName":"修改","BtnNo":"modify","BtnClass":null,"BtnIcon":"/icons/silkicons/application_edit.png","BtnScript":null,"MenuNo":"BaseManageProducts","InitStatus":null,"SeqNo":null},
     {"BtnID":99,"BtnName":"删除","BtnNo":"delete","BtnClass":null,"BtnIcon":"/icons/silkicons/delete.png","BtnScript":null,"MenuNo":"BaseManageProducts","InitStatus":null,"SeqNo":null},
     {"BtnID":100,"BtnName":"查看","BtnNo":"view","BtnClass":null,"BtnIcon":"/icons/silkicons/application_view_detail.png","BtnScript":null,"MenuNo":"BaseManageProducts","InitStatus":null,"SeqNo":null}
     ];
    ...
    LG.loadToolbar = function (grid, toolbarBtnItemClick) {
        ...
        LG.ajax({
            url: GLOBAL_CTX + '/user/getAuthorisedButton',
            loading:'正在加载工具条中...',
    ...
    LG.closeAndReloadParentWithTabId = function (tabid, parentTabid) {
        LG.closeCurrentTab(tabid);
        var iframe = window.frames[parentTabid];
        if (!iframe) {
            return;
