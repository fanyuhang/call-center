...
//        if (isNumber) o.type = "number";
        //changed by Allen 2012-0929
        if (isNumber) o.type = this.type;
    ...
    function show()
    {
        ...
        else
        {
			...
			{ 
				grid.select(rowdata);
				toSelect();
                if(options.handleSelect!=null)
                    options.handleSelect(grid.selected);
				win.hide();
			});
            ...
            win = $.ligerDialog.open({
                ...
                target: panle,
                buttons: [
                 { text: '选择', onclick: function (item, dialog) { toSelect();if(options.handleSelect!=null) options.handleSelect(grid.selected);dialog.hide(); } },
                 { text: '取消', onclick: function (item, dialog) { dialog.hide(); } }
                ]
            });
            if (options.search)
            {
                ...
                LG.addSearchButtons(formPanle, grid, options.searchBtn == false ? false : containerBtn1, options.advSearchBtn == false ? false : containerBtn2);
            }
            else
            {
                formPanle.remove();
